            ...
            try:
                ...
                for tls_id in tls_ids:
                    ...
                    try:
                        ...
                        print(
                            f"Error collecting queue lengths for TLS {tls_id} at step {step}: {e}"
                        )
                        traceback.print_exc()

                    # Adaptive or fixed timing logic
                    total_queue = sum(queue_lengths.values())
                    try:
                        if total_queue > QUEUE_THRESHOLD:
                            ...
                            for phase in fixed_phases[tls_id]:
                                ...
                                if (
                                    "y" in state
                                ):  # Yellow phase - keep original duration
                        ...
                        print(
                            f"Error in adaptive/fixed timing logic for TLS {tls_id} at step {step}: {e}"
                        )
                        traceback.print_exc()
                ...
                # Print dictionary values for testing/debugging
                print(f"Step {step} - Queue Data: {step_queue_data}")
                print(f"Step {step} - Speed Data: {step_speed_data}")
...
# A function that writes to the real time data to csf files
